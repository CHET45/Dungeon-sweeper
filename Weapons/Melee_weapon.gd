...
func Random_stats_generator():
	...
	if rng.randi_range(1,10)>8:
		chplus=rng.randi_range(1,4)
	elif rng.randi_range(1,9)>7:
		chminus=rng.randi_range(-1,-4)
	atk_speed=rng.randi_range(ceil(atk_speed-2+chminus),ceil(atk_speed+2+chplus))
